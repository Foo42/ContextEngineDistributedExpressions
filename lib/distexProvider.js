...
function createContract(requestId, expression, token, distexExchange) {
    var contract = new EventEmitter;

    contract.requestId = requestId;
    contract.expression = expression;
    contract.status = 'offered';
    contract.handlingToken = token;
    contract.pushEvent = function (eventToPush) {
        distexExchange.publish(token + '.event', eventToPush);
    };
...
function create(connection, userSuppliedCanHandleCallback) {
    ...
    function offerToHandle(messageObject) {
        ...
        myTokens[token] = createContract(messageObject.id, messageObject.expression, token, distexExchange);
        ...
        queue.topicEmitter.on(token + '.#', function (message, headers, deliveryInfo) {
            ...
            if (messageType === 'accept') {
                ...
                distexExchange.publish(contract.handlingToken + '.notWatching', {
                    requestId: contract.requestId
                });
            }
        });
